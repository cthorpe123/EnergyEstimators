  ...
  for(bool smeared : {false,true}){
    ...
    for(size_t i_f=0;i_f<Generators_v.size();i_f++){
      ...
      for(size_t i_e=0;i_e<estimators_str.size();i_e++){
        ...
        TH3D* h = (TH3D*)f->Get((gen+"_TrueEnergy_RecoEnergy_"+var+s+"_"+est).c_str()); 
        ...
        for(int i_dm=1;i_dm<h_fit_results->GetNbinsX()+1;i_dm++){
          ...
          if(makeloadsofplots){
            ...
            c->Print(("Plots/" + var + "/FitPlots/" + gen + "/" + est + "/" + "Point_" + point + "_" + gen + s + "_" + est +".png").c_str());
            p_plot->Clear();
            l->Clear();
          } 
      ...
      c->Print(("Plots/"+var+"/"+"DeltaM2FitResults_"+gen+s+".pdf").c_str());
      p_plot->Clear();
      l->Clear();
