  ...
  for(size_t i_f=0;i_f<InputFiles_v.size();i_f++){
    ...
    for(size_t i_v=0;i_v<vars.size();i_v++){      
      ...
      for(size_t i_e=0;i_e<kMAX;i_e++){
        ...
        h_EnergyBias.at(i_e).at(i_f).at(i_v) = new TH2D((gen+"_EnergyBias_"+var+"_"+estimator).c_str(),"",100,0,2,points.at(i_v),binning_a.at(i_v));
    ...
    for(Long64_t ievent=0;ievent<t->GetEntries();ievent++){
      ...
      std::vector<double> values = {W,nu_e,angle,missing_e,static_cast<double>(neutrons)};
      ...
      for(size_t i_v=0;i_v<vars.size();i_v++){
        ...
        for(int i_e=0;i_e<kMAX;i_e++){
          ...
          h_EnergyBias.at(i_e).at(i_f).at(i_v)->Fill(bias,val,weight);  
