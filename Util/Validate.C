...
void Validate(){
  ...
  for(size_t i_f=0;i_f<InputFiles_v.size();i_f++){
    ...
    h_NeutrinoEnergy.push_back(new TH1D((generator+"_NeutrinoEnergy").c_str(),"",100,0.0,7.0));
    h_W.push_back(new TH1D((generator+"_W").c_str(),"",100,0.0,5.0));
    h_LeptonMom.push_back(new TH1D((generator+"_LeptonMom").c_str(),"",100,0.0,6.0));
  ...
  THStack* hs_W = new THStack("hs",";W (GeV);Shape");
  THStack* hs_LeptonMom = new THStack("hs",";Lepton Momentum (GeV);Shape");
  THStack* hs_LeptonAngle = new THStack("hs",";Lepton Cos(#theta);Shape");
