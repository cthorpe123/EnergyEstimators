...
void FunWithNeutrons(){
  ...
  for(size_t i_f=0;i_f<InputFiles_v.size();i_f++){
    ...
    for(int i_e=0;i_e<estimators_str.size();i_e++){
      ...
      h_Change_PerfectNeutrons.at(i_e).push_back(new TH1D((generator+"_Change_PerfectNeutrons_"+est).c_str(),"",200,-0.5,0.8));
      h_Change_SmearedNeutrons.at(i_e).push_back(new TH1D((generator+"_Change_SmearedNeutrons_"+est).c_str(),"",200,-0.5,0.8));
    }
    ...
    for(Long64_t ievent=0;ievent<t->GetEntries();ievent++){
      ...
      for(int i_p=0;i_p<pdg->size();i_p++)
        if(pdg->at(i_p) == 2112 && p4->at(i_p).Vect().Mag() > 0.3) has_neutron = true;
