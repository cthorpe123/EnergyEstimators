...
void DeltaM2Plot(){
  ...
  std::vector<std::string> labels2 = {"","Perfect","Smeared"};

  for(size_t i_f=0;i_f<Generators_v.size();i_f++){
    std::string gen = Generators_v.at(i_f);    
    std::vector<std::vector<TGraph*>> g_fit_results(labels.size());
    std::string axis_title; 

    for(size_t i_l=0;i_l<labels.size();i_l++){
      std::string label = labels.at(i_l);

      double min_fit_ratio = 1;
      double max_fit_ratio = 1;

      g_fit_results.at(i_l).resize(kMAX);
      ...
      for(size_t i_e=0;i_e<estimators_str.size();i_e++){
        ...
        g_fit_results.at(i_l).at(i_e) = new TGraph(x_fit_results.size(),&(x_fit_results[0]),&(y_fit_results[0]));
    ...
    for(size_t i_l=0;i_l<labels.size();i_l++){
      for(size_t i_e=0;i_e<estimators_str.size();i_e++){
        std::string est = estimators_str.at(i_e);
        if(var == "W" && i_e == kSFMethod) continue;
        g_fit_results.at(i_l).at(i_e)->SetLineColor(colors.at(i_e)); 
        g_fit_results.at(i_l).at(i_e)->SetLineWidth(2); 
        g_fit_results.at(i_l).at(i_e)->SetLineStyle(2*i_l+1); 
        mg->Add(g_fit_results.at(i_l).at(i_e));
        l->AddEntry(g_fit_results.at(i_l).at(i_e),(est+" "+labels2.at(i_l)).c_str(),"L");
      }
    }

    mg->Draw("AL"); 
    l->Draw();
    c->Print(("Plots/DeltaM2FitResults_"+var+"_"+gen+".pdf").c_str());
    c->Clear();
    l->Clear();
