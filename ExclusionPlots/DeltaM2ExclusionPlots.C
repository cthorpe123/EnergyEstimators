...
void DeltaM2ExclusionPlots(){
  ...
  std::vector<std::vector<TH1D*>> h_RecoEnergy_DeltaMSqCV(estimators_tmp.size());
  std::vector<std::vector<TH1D*>> h_RecoEnergy_DeltaMSq_Plus(estimators_tmp.size());
  ...
  for(size_t i_e=0;i_e<estimators_tmp.size();i_e++){
    ...
    for(std::string generator : Generators_v){
      h_RecoEnergy_DeltaMSqCV.at(i_e).push_back(static_cast<TH1D*>(f_in->Get((generator+"_RecoEnergy_DeltaMSqCV_"+estimator).c_str())));
      h_RecoEnergy_DeltaMSq_Plus.at(i_e).push_back(static_cast<TH1D*>(f_in->Get((generator+"_RecoEnergy_DeltaMSq_Plus_"+estimator).c_str())));
      h_RecoEnergy_DeltaMSq_Minus.at(i_e).push_back(static_cast<TH1D*>(f_in->Get((generator+"_RecoEnergy_DeltaMSq_Minus_"+estimator).c_str())));
    }
  }   
  ...
  for(size_t i_e=0;i_e<estimators_tmp.size();i_e++){
    ...
    for(size_t i_f=0;i_f<Generators_v.size();i_f++){
      ...
      const TH1D* h_unscaled_deltam2cv = h_RecoEnergy_DeltaMSqCV.at(i_e).at(i_f); 
      const TH1D* h_unscaled_plusdeltam2 = h_RecoEnergy_DeltaMSq_Plus.at(i_e).at(i_f); 
      const TH1D* h_unscaled_minusdeltam2 = h_RecoEnergy_DeltaMSq_Minus.at(i_e).at(i_f); 
  ...
  THStack* hs_bands_plus = new THStack("hs_bands_plus","#Delta m^{2}_{23}=CV Exclusion at #Delta m^{2}_{23}=CV+3#sigma;Exposure (KT x 10^{21} POT);sqrt(#chi^{2}/ndof)");
  THStack* hs_lines_plus = new THStack("hs_lines_plus","#Delta m^{2}_{23}=CV Exclusion at #Delta m^{2}_{23}=CV+3#sigma;Exposure (KT x 10^{21} POT);sqrt(#chi^{2}/ndof)");
  THStack* hs_bands_minus = new THStack("hs_bands_minus","#Delta m^{2}_{23}=CV Exclusion at #Delta m^{2}_{23}=CV-3#sigma;Exposure (KT x 10^{21} POT);sqrt(#chi^{2}/ndof)");
